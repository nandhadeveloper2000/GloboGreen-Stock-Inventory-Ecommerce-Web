...
export default function CreatePartyAccountPage() {
  ...
  async function handleSubmit() {
    ...
    if (gstNumber.trim() && !gstState.trim()) {
    ...
    if (!Number.isFinite(openingBalance) || openingBalance < 0) {
      nextErrors.openingBalance = "Opening balance must be zero or greater.";
    }

    if (!Number.isFinite(creditLimit) || creditLimit < 0) {
      nextErrors.creditLimit = "Credit limit must be zero or greater.";
    }
      ...
      <div className="mx-auto w-full max-w-5xl space-y-6">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-2 mt-4">
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-slate-700">GST State</label>
              <input
                type="text"
                value={gstState}
                onChange={(event) => setGstState(event.target.value)}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-sky-400"
                placeholder="Enter GST state"
              />
              {errors.gstState ? <p className="mt-1 text-xs text-rose-600">{errors.gstState}</p> : null}
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 mt-4">
            <div>
              ...
              <input
                ...
                placeholder="0"
              />
              {errors.openingBalance ? <p className="mt-1 text-xs text-rose-600">{errors.openingBalance}</p> : null}
            </div>
            <div>
              ...
              <input
                ...
                placeholder="0"
              />
              {errors.creditLimit ? <p className="mt-1 text-xs text-rose-600">{errors.creditLimit}</p> : null}
            </div>
          </div>
