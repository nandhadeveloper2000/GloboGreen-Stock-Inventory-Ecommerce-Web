...
import { ArrowLeft, Check, ChevronDown, CreditCard, Loader2, Plus, Search, Trash2, UserPlus, X } from "lucide-react";
...
export default function SalesCreatePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const allCustomers = Array.isArray(customerResult.data) ? customerResult.data : [];
        const nextCustomers = allCustomers.filter((customer) => customer.isActive !== false || customer.isWalkIn);
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-2 md:p-3">
      <div className="mx-auto w-full max-w-9xl space-y-2">
          ...
          <div className="flex flex-wrap items-end gap-3 xl:flex-nowrap">
            <div className="min-w-50 flex-2">
              <div className="mb-1 flex items-center justify-between gap-2">
                ...
                <button type="button" onClick={() => setShowCustomerModal(true)} className="inline-flex h-6 items-center gap-1 rounded border border-slate-200 bg-white px-2 text-[11px] font-bold text-[#00008b] hover:bg-slate-50"><UserPlus className="h-3 w-3" /> Add Customer</button>
              </div>
              ...
              <div className="mt-1 flex gap-3 text-[11px] font-semibold">
                <span className="text-slate-500">Points: <b className="text-slate-800">{selectedCustomer?.points || 0}</b></span>
                <span className="text-slate-500">Due Balance: <b className="text-rose-600">{money(selectedCustomer?.dueBalance || 0)}</b></span>
              </div>
            </div>
            <div className="min-w-37.5 flex-1">
              <label className="mb-1 block text-xs font-bold text-slate-700">Salesman</label>
              <input value={salesmanName} onChange={(event) => setSalesmanName(event.target.value)} placeholder="None" className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" />
            </div>
            <div className="min-w-35 flex-1">
              <label className="mb-1 block text-xs font-bold text-slate-700">Payment Mode</label>
              <select value={paymentMethod} onChange={(event) => setPaymentMethod(normalizePaymentMethod(event.target.value))} className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-medium outline-none focus:border-[#00008b]">
                {PAYMENT_METHODS.map((method) => <option key={method} value={method}>{method.replace(/_/g, " ")}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2 pb-1">
              <div className="flex h-9 items-center justify-center rounded-full bg-sky-100 px-4 text-sm font-black text-sky-700">Items: {totals.itemCount}</div>
              <div className="flex h-9 items-center justify-center rounded-full bg-sky-100 px-4 text-sm font-black text-sky-700">Qty: {totals.totalQty}</div>
            </div>
            <div className="ml-auto flex flex-col items-end justify-end pb-1">
              ...
              <p className="text-4xl font-black leading-none text-red-600">{totals.netAmount.toFixed(2)}</p>
            </div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-345 border-collapse text-left text-[13px]">
                ...
                {rows.map((row, index) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b border-slate-200 bg-[#fff9d6] align-top">
                      ...
                      <td className="border-r border-slate-200 px-2 py-2"><div data-product-combobox="true" className="relative min-w-42.5"><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input value={row.itemCode} onFocus={() => setOpenProductRowId(row.id)} onChange={(event) => { updateRow(row.id, { itemCode: event.target.value, productName: "", productId: "", shopProductId: "" }); setOpenProductRowId(row.id); }} placeholder="scan or type..." className="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-8 text-sm outline-none focus:border-[#00008b]" /><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /></div>{isProductOpen ? <div className="absolute left-0 top-[calc(100%+0.35rem)] z-30 w-90 max-w-[72vw] overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl"><div className="max-h-72 overflow-y-auto p-1.5">{filteredProducts.length ? filteredProducts.map((product) => <button key={product._id} type="button" onClick={() => selectProduct(row.id, product._id)} className="flex w-full items-start justify-between gap-3 rounded-md px-3 py-2 text-left text-slate-700 hover:bg-slate-50"><div className="min-w-0"><p className="truncate text-[13px] font-bold">{getShopProductName(product)}</p><p className="mt-0.5 text-[11px] text-slate-500">SKU: {getShopProductCode(product) || "-"} | Stock: {toNumber(product.qty, 0)} | Rate: {money(getShopProductSellingPrice(product))}</p></div>{String(row.shopProductId) === String(product._id) ? <Check className="mt-0.5 h-4 w-4 shrink-0" /> : null}</button>) : <p className="px-3 py-3 text-[13px] text-slate-500">No matching product found.</p>}</div></div> : null}</div></td>
                      <td className="border-r border-slate-200 px-2 py-2"><div className="flex h-9 min-w-60 items-center rounded-md border border-slate-300 bg-white px-3 text-sm font-bold text-slate-700">{row.productName || "-"}</div></td>
                      <td className="border-r border-slate-200 px-2 py-2"><input value={row.batch} onChange={(event) => updateRow(row.id, { batch: event.target.value })} placeholder="No batch" className="h-9 min-w-32.5 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" /></td>
                      <td className="border-r border-slate-200 px-2 py-2"><input readOnly value={Number(row.mrp || 0).toFixed(2)} className="h-9 w-25 rounded-md border border-slate-300 bg-slate-100 px-3 text-sm font-bold outline-none" /></td>
                      <td className="border-r border-slate-200 px-2 py-2"><input type="number" min={1} value={row.qty} onChange={(event) => updateRowWithPercentDiscount(row.id, { qty: event.target.value })} className="h-9 w-20 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" /></td>
                      <td className="border-r border-slate-200 px-2 py-2"><input readOnly value={row.unit || "Pcs"} className="h-9 w-23.75 rounded-md border border-slate-300 bg-slate-100 px-3 text-sm font-bold outline-none" /></td>
                      <td className="border-r border-slate-200 px-2 py-2"><input type="number" min={0} step="0.01" value={row.price} onChange={(event) => updateRowWithPercentDiscount(row.id, { price: event.target.value })} className="h-9 w-27.5 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" /></td>
                      <td className="border-r border-slate-200 px-2 py-2"><select value={row.taxPercent} onChange={(event) => updateRow(row.id, { taxPercent: event.target.value })} className="h-9 min-w-28.75 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]">{GST_OPTIONS.map((option) => <option key={option.label} value={option.percent}>{option.label}</option>)}</select></td>
                      <td className="border-r border-slate-200 px-2 py-2"><div className="grid min-w-42.5 grid-cols-2 gap-2"><input type="number" min={0} step="0.01" value={row.discountPercent} onChange={(event) => updateRowDiscountPercent(row.id, event.target.value)} className="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" /><input type="number" min={0} step="0.01" value={row.discountAmount} onChange={(event) => updateRowDiscountAmount(row.id, event.target.value)} className="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" /></div></td>
        ...
        <section className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <div className="rounded-md border border-slate-200 bg-white p-3 shadow-sm">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Disc %</label>
                <input type="number" min={0} step="0.01" value={orderDiscount} onChange={(event) => setOrderDiscount(event.target.value)} className="h-9 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#00008b]" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Discount Amt</label>
                <input type="number" readOnly value={toNumber(orderDiscount, 0)} className="h-9 w-full rounded-md border border-slate-200 bg-slate-50 px-3 text-sm outline-none" />
              </div>
            </div>
            <div className="mt-3 space-y-1.5 border-t border-slate-100 pt-3 text-sm">
              <div className="flex items-center justify-between"><span className="text-slate-500">Subtotal:</span><b className="text-slate-900">{money(totals.subtotal)}</b></div>
              <div className="flex items-center justify-between"><span className="text-slate-500">Tax:</span><b className="text-slate-900">{money(totals.tax)}</b></div>
              <div className="flex items-center justify-between"><span className="text-slate-500">Discount:</span><b className="text-rose-600">-{money(totals.discount)}</b></div>
            </div>
            <div className="mt-3 rounded-md border border-[#00008b]/20 bg-[#00008b]/5 px-4 py-3 text-right">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#00008b]">Net Amount:</p>
              <p className="mt-0.5 text-4xl font-black text-[#00008b]">{totals.netAmount.toFixed(2)}</p>
            </div>
            <div className="mt-3 grid grid-cols-3 gap-1.5">
              <button type="button" onClick={() => void finishDirectBill("CASH")} disabled={saving} className="h-9 rounded-md border border-[#00008b]/20 bg-[#00008b]/5 text-[11px] font-black text-[#00008b] disabled:opacity-60">[Space] Cash</button>
              <button type="button" onClick={() => setShowSplitPopup(true)} disabled={saving} className="h-9 rounded-md border border-amber-200 bg-amber-50 text-[11px] font-black text-amber-700 disabled:opacity-60">[F2] Split</button>
              <button type="button" onClick={() => void finishDirectBill("CARD")} disabled={saving} className="h-9 rounded-md border border-purple-200 bg-purple-50 text-[11px] font-black text-purple-700 disabled:opacity-60">[F3] Card</button>
              <button type="button" onClick={() => void finishDirectBill("UPI")} disabled={saving} className="h-9 rounded-md border border-rose-200 bg-rose-50 text-[11px] font-black text-rose-700 disabled:opacity-60">[F4] UPI</button>
              <button type="button" onClick={() => void finishDirectBill("CREDIT")} disabled={saving} className="h-9 rounded-md border border-cyan-200 bg-cyan-50 text-[11px] font-black text-cyan-700 disabled:opacity-60">[F5] Credit</button>
              <button type="button" onClick={() => setShowSalesHistoryPopup(true)} className="h-9 rounded-md border border-slate-300 bg-slate-50 text-[11px] font-black text-slate-700">[Ctrl+V] View</button>
            </div>
            <button type="button" onClick={() => void finishDirectBill(paymentMethod)} disabled={saving} className="mt-2 inline-flex h-11 w-full items-center justify-center gap-2 rounded-md bg-emerald-600 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-60">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <CreditCard className="h-5 w-5" />} BILL / PRINT</button>
          </div>
        </section>
      </div>
