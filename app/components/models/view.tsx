...
export default function MyModelViewPage({
  ...
  const handleEdit = () => {
    ...
    router.push(`/shopowner/models/edit/${editId}`);
  };
  ...
  const content = loading ? (
    ...
    <div className={showAsModal ? "px-6 py-6" : ""}>
      <div className="rounded-3xl border border-rose-200 bg-rose-50 p-5 text-sm font-semibold text-rose-700">
        <p className="text-base font-black">Model mapping not found</p>

        <p className="mt-1">
          The selected shop model record could not be loaded.
        </p>
      </div>
    </div>
  ) : (
          ...
          <div className="space-y-4">
            <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                <Wrench className="h-4.5 w-4.5 text-slate-700" />
  ...
  if (showAsModal) {
    return (
      ...
      >
        <div
          className="relative z-10 my-4 w-full max-w-4xl overflow-hidden round  ed-card border border-slate-200 bg-white shadow-2xl"
