...
import { splitDynamicFieldValuesForSubmit } from "@/lib/product-type-fields";
...
export function buildProductFormData(
  ...
  const dynamicFieldPayload = splitDynamicFieldValuesForSubmit(
  ...
  formData.append("externalSourceUrl", payload.externalSourceUrl);
  formData.append("externalSourceType", payload.externalSourceType);
  formData.append("externalProductId", payload.externalProductId);
  formData.append("externalProductIdType", payload.externalProductIdType);
  ...
  formData.append(
    "dynamicFieldValues",
    JSON.stringify(dynamicFieldPayload.values),
  );
  formData.append(
    "dynamicFieldFileUploads",
    JSON.stringify(
      dynamicFieldPayload.uploads.map((item) => ({
        sectionHeadingId: item.sectionHeadingId,
        groupId: item.groupId,
        fieldId: item.fieldId,
        key: item.key,
        uploadField: item.uploadField,
        itemIndex: item.itemIndex,
        matrixComboKey: item.matrixComboKey,
        matrixFileKey: item.matrixFileKey,
      })),
    ),
  );
  formData.append("isActive", String(payload.isActive));

  dynamicFieldPayload.uploads.forEach((item) => {
    formData.append(item.uploadField, item.file);
  });
