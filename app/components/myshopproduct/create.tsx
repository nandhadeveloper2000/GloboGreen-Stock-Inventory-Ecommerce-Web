...
export default function CreateMyShopProductPage({
  ...
  const [shopCategoryMaps, setShopCategoryMaps] = useState<ShopCategoryMapItem[]>([]);
  const [shopSubCategoryMaps, setShopSubCategoryMaps] = useState<
    ShopSubCategoryMapItem[]
  >([]);
  const [shopBrandMaps, setShopBrandMaps] = useState<ShopBrandMapItem[]>([]);
  const [shopModelMaps, setShopModelMaps] = useState<ShopModelMapItem[]>([]);
  ...
  const isWholesaleShop = useMemo(
    () => normalizeShopType(selectedShopType) === "WHOLESALE_SHOP",
    [selectedShopType]
  );
  ...
  const filterCategoryOptions = useMemo(() => {
    ...
    for (const item of catalogItems) {
      const id = getReferenceId(item.categoryId);
      if (id && !seen.has(id)) {
        seen.set(id, getReferenceDisplayName(item.categoryId) || getReferenceName(item.categoryId) || id);
      }
    }
  ...
  const filterSubCategoryOptions = useMemo(() => {
    const items = filterCategoryId
      ? catalogItems.filter((item) => getReferenceId(item.categoryId) === filterCategoryId)
      : catalogItems;
    const seen = new Map<string, string>();
    for (const item of items) {
      const id = getReferenceId(item.subcategoryId);
      if (id && !seen.has(id)) {
        seen.set(id, getReferenceDisplayName(item.subcategoryId) || getReferenceName(item.subcategoryId) || id);
      }
    }
    return Array.from(seen.entries()).map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name));
  }, [catalogItems, filterCategoryId]);

  const filterBrandOptions = useMemo(() => {
    let items = filterCategoryId
      ? catalogItems.filter((item) => getReferenceId(item.categoryId) === filterCategoryId)
      : catalogItems;
    if (filterSubCategoryId) {
      items = items.filter((item) => getReferenceId(item.subcategoryId) === filterSubCategoryId);
    }
    const seen = new Map<string, string>();
    for (const item of items) {
      for (const brand of toReferenceArray(item.brandId)) {
        const id = getReferenceId(brand);
        if (id && !seen.has(id)) {
          seen.set(id, getReferenceDisplayName(brand) || getReferenceName(brand) || id);
        }
      }
    }
    return Array.from(seen.entries()).map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name));
  }, [catalogItems, filterCategoryId, filterSubCategoryId]);
  ...
  const filterModelOptions = useMemo(() => {
    ...
    if (filterBrandId) {
      items = items.filter((item) =>
        toReferenceArray(item.brandId).some((b) => getReferenceId(b) === filterBrandId)
      );
    }
    const seen = new Map<string, string>();
    for (const item of items) {
      for (const model of toReferenceArray(item.modelId)) {
        const id = getReferenceId(model);
        if (id && !seen.has(id)) {
          seen.set(id, getReferenceDisplayName(model) || getReferenceName(model) || id);
        }
      }
    }
    return Array.from(seen.entries()).map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name));
  }, [catalogItems, filterCategoryId, filterSubCategoryId, filterBrandId]);

  const filteredCatalogItems = useMemo(() => {
    let items = catalogItems;
    if (filterCategoryId) {
      items = items.filter((item) => getReferenceId(item.categoryId) === filterCategoryId);
    }
    if (filterSubCategoryId) {
      items = items.filter((item) => getReferenceId(item.subcategoryId) === filterSubCategoryId);
    }
    ...
    if (filterModelId) {
      items = items.filter((item) =>
        toReferenceArray(item.modelId).some((m) => getReferenceId(m) === filterModelId)
      );
    }
    return items;
  }, [catalogItems, filterCategoryId, filterSubCategoryId, filterBrandId, filterModelId]);

  const filteredSearchItems = useMemo(() => {
    const q = productSearch.trim().toLowerCase();
    if (!q) return filteredCatalogItems;
    return filteredCatalogItems.filter((item) =>
      getCatalogProductLabel(item).toLowerCase().includes(q)
    );
  }, [filteredCatalogItems, productSearch]);

  const selectedProductItemCode = useMemo(() => {
    return (
      getShopProductItemCode(existingItem) || getProductSku(selectedCatalogProduct)
    );
  }, [existingItem, selectedCatalogProduct]);

  const selectedProductCatalogFields = useMemo(() => {
    if (!selectedCatalogProduct) return [];

    const categoryId = getReferenceId(selectedCatalogProduct.categoryId);
    const subCategoryId = getReferenceId(selectedCatalogProduct.subcategoryId);
    const brandReferences = toReferenceArray(selectedCatalogProduct.brandId);
    const modelReferences = toReferenceArray(selectedCatalogProduct.modelId);

    const categoryMap = shopCategoryMaps.find(
      (item) =>
        item.isActive !== false && getReferenceId(item.categoryId) === categoryId
    );
    const subCategoryMap = shopSubCategoryMaps.find(
      (item) =>
        item.isActive !== false &&
        getReferenceId(item.subCategoryId) === subCategoryId
    );
    ...
    const brandNames = Array.from(
      new Set(
        brandReferences
          .map((item) => {
            const referenceId = getReferenceId(item);
            const mappedItem = shopBrandMaps.find(
              (row) =>
                row.isActive !== false &&
                getReferenceId(row.brandId) === referenceId
            );

            return (
              getReferenceDisplayName(mappedItem?.brandId) ||
              getReferenceDisplayName(item) ||
              getReferenceName(mappedItem?.brandId) ||
    ...
    const modelNames = Array.from(
      new Set(
        modelReferences
          .map((item) => {
            const referenceId = getReferenceId(item);
            const mappedItem = shopModelMaps.find(
              (row) =>
                row.isActive !== false &&
                getReferenceId(row.modelId) === referenceId
            );

            return (
              getReferenceDisplayName(mappedItem?.modelId) ||
              getReferenceDisplayName(item) ||
              getReferenceName(mappedItem?.modelId) ||
              getReferenceName(item)
            );
          })
          .filter(Boolean)
    ...
    const categoryName =
      getReferenceDisplayName(categoryMap?.category) ||
      getReferenceDisplayName(categoryMap?.categoryId) ||
      getReferenceDisplayName(selectedCatalogProduct.categoryId) ||
      getReferenceName(categoryMap?.category) ||
      getReferenceName(categoryMap?.categoryId) ||
      getReferenceName(selectedCatalogProduct.categoryId) ||
      "-";

    const subCategoryName =
      getReferenceDisplayName(subCategoryMap?.subCategoryId) ||
      getReferenceDisplayName(selectedCatalogProduct.subcategoryId) ||
      getReferenceName(subCategoryMap?.subCategoryId) ||
      getReferenceName(selectedCatalogProduct.subcategoryId) ||
      "-";

    const fallbackBrandNames = getReferenceNames(selectedCatalogProduct.brandId);
    const fallbackModelNames = getReferenceNames(selectedCatalogProduct.modelId);
    ...
    return [
      ...
      {
        label: "My Brand",
        value: brandNames.join(", ") || fallbackBrandNames.join(", ") || "-",
      },
      {
        label: "My Model",
        value: modelNames.join(", ") || fallbackModelNames.join(", ") || "-",
      },
      {
        label: "Item Code",
  ...
  const loadShopMappings = useCallback(async () => {
    if (!accessToken || !selectedShopId || !productEntryShopSelected) {
      setShopCategoryMaps([]);
    ...
    const requests = [
      {
        ...
        url: SummaryApi.shopCategoryMapByShop.url(selectedShopId),
      },
      {
        method: SummaryApi.shopSubCategoryMapByShop.method,
        url: SummaryApi.shopSubCategoryMapByShop.url(selectedShopId),
      },
      {
        method: SummaryApi.shopBrandMapByShop.method,
        url: SummaryApi.shopBrandMapByShop.url(selectedShopId),
      },
      {
        method: SummaryApi.shopModelMapByShop.method,
        url: SummaryApi.shopModelMapByShop.url(selectedShopId),
      },
    ];
    ...
    setShopCategoryMaps(normalizedResults[0] as ShopCategoryMapItem[]);
    setShopSubCategoryMaps(normalizedResults[1] as ShopSubCategoryMapItem[]);
    setShopBrandMaps(normalizedResults[2] as ShopBrandMapItem[]);
    setShopModelMaps(normalizedResults[3] as ShopModelMapItem[]);
  }, [accessToken, productEntryShopSelected, selectedShopId]);
  ...
  const loadPageData = useCallback(async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        if (matchedProduct) {
          setProductSearch(getCatalogProductLabel(matchedProduct));
        }
      ...
      setExistingItem(null);
      setForm(buildCreateFormState(null));
      setVendorSearch("");
      setProductSearch("");
    } catch (error) {
      setCatalogItems([]);
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (!target?.closest("[data-product-combobox='true']")) {
  ...
  const handleFilterCategoryChange = useCallback((id: string) => {
    ...
    setExistingItem(null);
    setProductSearch("");
    setForm((previous) => ({ ...buildCreateFormState(null), vendorId: previous.vendorId }));
  }, []);

  const handleFilterSubCategoryChange = useCallback((id: string) => {
  ...
  const handleFilterBrandChange = useCallback((id: string) => {
    setFilterBrandId(id);
    setFilterModelId("");
    setExistingItem(null);
    setProductSearch("");
  ...
  const handleFilterModelChange = useCallback((id: string) => {
    setFilterModelId(id);
    setExistingItem(null);
    setProductSearch("");
    setForm((previous) => ({ ...buildCreateFormState(null), vendorId: previous.vendorId }));
  ...
  const handleProductSelection = useCallback(
    (nextProductId: string) => {
      ...
      if (product) {
        setProductSearch(getCatalogProductLabel(product));
      }
      setProductSearchOpen(false);
    },
    [catalogItems]
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedShopId || !productEntryShopSelected) {
      toast.error(
        "Please select a valid Main Shop, Branch Shop, or Wholesale Shop."
      );
      return;
    }
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-amber-800">
              Only Main Shop, Branch Shop, or Wholesale Shop can add shop products.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <section className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        ...
        {!isEditMode && filterCategoryOptions.length > 0 ? (
          <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
            <div>
              <FieldLabel htmlFor="filterCategory" label="Category" />
              <select
                id="filterCategory"
                value={filterCategoryId}
                disabled={disabledForm}
                onChange={(event) => handleFilterCategoryChange(event.target.value)}
            ...
            <div>
              <FieldLabel htmlFor="filterSubCategory" label="Subcategory" />
              <select
                id="filterSubCategory"
                value={filterSubCategoryId}
                disabled={disabledForm || filterSubCategoryOptions.length === 0}
                onChange={(event) => handleFilterSubCategoryChange(event.target.value)}
                className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-fuchsia-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
              >
            ...
            <div>
              <FieldLabel htmlFor="filterBrand" label="Brand" />
              <select
                id="filterBrand"
                value={filterBrandId}
                disabled={disabledForm || filterBrandOptions.length === 0}
                onChange={(event) => handleFilterBrandChange(event.target.value)}
                className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-fuchsia-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
              >
                <option value="">All Brands</option>
                {filterBrandOptions.map((opt) => (
                  <option key={opt.id} value={opt.id}>{opt.name}</option>
                ))}
              </select>
            </div>
            ...
            {filterModelOptions.length > 0 ? (
              <div>
                <FieldLabel htmlFor="filterModel" label="Model" />
                <select
                  id="filterModel"
                  value={filterModelId}
                  disabled={disabledForm}
                  onChange={(event) => handleFilterModelChange(event.target.value)}
                  className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-900 shadow-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-fuchsia-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                >
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <div data-product-combobox="true" className="relative">
            <FieldLabel htmlFor="productSearch" label="Product Name" />
            <div className="relative">
              ...
              <input
                ...
                onFocus={() => !isEditMode && setProductSearchOpen(true)}
                onChange={(event) => {
            ...
            {form.productId && selectedCatalogProduct ? (
              <p className="mt-1 text-[11px] font-semibold text-emerald-700">
                Selected: {getCatalogProductLabel(selectedCatalogProduct)}
              </p>
            ) : null}

            {productSearchOpen && !isEditMode ? (
                ...
                {filteredSearchItems.length > 0 ? (
                  filteredSearchItems.map((item) => (
                    <button
                      key={item._id}
                      type="button"
                      onClick={() => handleProductSelection(item._id)}
                    ...
                    >
                      <span>{getCatalogProductLabel(item)}</span>
                      {form.productId === item._id ? (
        ...
        {selectedCatalogProduct ? (
          <div className="mt-5 space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
              {selectedProductCatalogFields.map((field) => (
                <div
                  key={field.label}
            ...
            <p className="text-[11px] font-medium text-slate-500">
              These product mapping fields are display only and follow the selected
              shop catalog.
            </p>
          </div>
        ) : null}
          ...
          <div className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-[12px] font-medium text-slate-500">
                  Main and branch shops use single pricing. Wholesale shop also supports bulk pricing.
                </p>
              </div>
