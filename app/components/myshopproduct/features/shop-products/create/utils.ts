...
export function normalizeShopType(value?: string | null) {
...
export function getProductSku(
  ...
  return String(product.sku || product.itemModelNumber || "").trim().toUpperCase();
}

export function getShopProductItemCode(item?: ShopProductItem | null) {
  const directCode = String(
    item?.itemCode || item?.sku || item?.itemModelNumber || ""
  )
    .trim()
    .toUpperCase();
  ...
  return (
    getProductSku(product) || String(product.itemKey || item?.itemKey || "").trim()
  );
}

export function getReferenceId(value?: NamedReference | null) {
  if (!value) return "";
...
export function getReferenceName(value?: NamedReference | null) {
  if (!value) return "";
  if (typeof value === "string") return value;

  return String(value.name || value.nameKey || "").trim();
}

export function getReferenceDisplayName(value?: NamedReference | null) {
...
export function toReferenceArray(
  value?: NamedReference | NamedReference[] | null
) {
  if (!value) return [];
...
export function getReferenceNames(
  ...
  return toReferenceArray(value).map((item) => getReferenceName(item)).filter(Boolean);
}
