...
function getCategoryName(category: CategoryRef | undefined) {
  if (!category) return "";
  if (typeof category === "string") return "";
  return category.name || "";
}

function getMasterCategoryName(category: CategoryRef | undefined) {
  if (!category || typeof category === "string") return "";

  const master = category.masterCategoryId;
...
export default function MySubCategoryCreatePage() {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      const response = await apiClient({
        method: SummaryApi.sub_category_list.method,
      ...
      const rows: SubCategoryItem[] = response.data?.data || [];

      setSubCategories(rows);
      setSelectedIds([]);
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "Failed to load subcategories"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const availableSubCategories = useMemo(() => {
    ...
    return subCategories.filter((item) => {
      ...
      const masterCategoryName = getMasterCategoryName(item.categoryId);

      return `${item.name || ""} ${categoryName} ${masterCategoryName}`
        .toLowerCase()
        .includes(keyword);
    });
  ...
  const selectAllVisible = () => {
    setSelectedIds(availableSubCategories.map((item) => item._id));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient({
        method: SummaryApi.shopSubCategoryMapBulkCreate.method,
        url: SummaryApi.shopSubCategoryMapBulkCreate.url,
        data: {
      ...
      toast.success("Subcategories mapped successfully");
      router.push("/shopowner/my-subcategory/list");
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "Failed to map subcategories"));
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-4 md:p-6">
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-800">
          Please select a shop before creating my subcategories.
        </div>
      </div>
    );
  }
          ...
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-white/80">
                Select the active subcategories your shop can use for shop
                products.
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={() => router.push("/shopowner/my-subcategory/list")}
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="relative w-full md:max-w-md">
              ...
              <input
                ...
                placeholder="Search subcategory, category, or master category..."
                  ...
                  <tbody>
                    {availableSubCategories.map((item, index) => {
                      ...
                      const masterCategoryName =
                        getMasterCategoryName(item.categoryId) || "-";
