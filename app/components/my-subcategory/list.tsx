...
function getMasterCategory(category: CategoryData | null): MasterCategoryData | null {
...
function getMasterCategoryName(row: ShopSubCategoryMap) {
  return getMasterCategory(getCategory(getSubCategory(row)))?.name || "-";
}
...
export default function MySubCategoryListPage() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const api = SummaryApi.shopSubCategoryMapByShop;
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const masterCategoryName = getMasterCategoryName(row);

      const matchesSearch =
        !keyword ||
        subCategoryName.toLowerCase().includes(keyword) ||
        categoryName.toLowerCase().includes(keyword) ||
        masterCategoryName.toLowerCase().includes(keyword);
  ...
  async function handleDelete(id: string) {
    try {
      ...
      const api = SummaryApi.shopSubCategoryMapDelete;
      ...
      <div className="mx-auto max-w-6xl space-y-4">
          ...
          <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-white/85">
                Manage subcategories mapped to {shopName}.
              </p>
            </div>

            <button
              type="button"
              onClick={() => router.push("/shopowner/my-subcategory/create")}
          ...
          {loading ? (
            ...
            <div className="flex min-h-65 flex-col items-center justify-center px-4 text-center">
              ...
              <p className="mt-1 max-w-md text-sm font-medium text-slate-500">
                Add subcategories to make them available for this shop.
              </p>
            </div>
          ) : (
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2">
                            <button
                              type="button"
                              onClick={() =>
                                router.push(`/shopowner/my-subcategory/view?id=${row._id}`)
                              }
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                router.push(`/shopowner/my-subcategory/edit/${row._id}`)
                              }
        ...
        {!loading && filteredRows.length > 0 ? (
          <div className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-600 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <span>
              Showing {filteredRows.length} of {rows.length} subcategories
            </span>
