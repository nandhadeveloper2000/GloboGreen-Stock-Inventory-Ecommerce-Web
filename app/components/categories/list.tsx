...
import MyCategoryCreatePage from "./create";
import MyCategoryEditPage from "./edit";
import MyCategoryViewPage from "./view";
...
export default function MyCategoryListPage() {
  ...
  const [categoryFormModal, setCategoryFormModal] =
    useState<CategoryFormModalState>(null);
  ...
  const openEditCategoryModal = useCallback((mapId: string) => {
    setCategoryFormModal({
      mode: "edit",
      mapId,
    });
  ...
  const closeCategoryFormModal = useCallback(() => {
  ...
  useEffect(() => {
    if (!categoryFormModal && !viewingCategoryId) return;
  ...
  }, [categoryFormModal, viewingCategoryId]);

  async function handleToggleStatus(row: ShopCategoryMap) {
    try {
      setTogglingId(row._id);

      const api = SummaryApi.shopCategoryMapToggleActive;

      const url =
        typeof api.url === "function"
          ? api.url(row._id)
          : `/api/shop-category-maps/${row._id}/toggle-active`;
      ...
      toast.success(
        updatedStatus ? "Category activated" : "Category deactivated"
      );
    } catch (error: unknown) {
  ...
  function handleDelete(row: ShopCategoryMap) {
    const categoryName = getCategory(row)?.name || "this category";

    toast(`Remove ${categoryName}?`, {
      description: `This will remove the category mapping from ${
        shopName || "this shop"
      }.`,
      action: {
  ...
  return (
    <div className="page-shell">
        ...
        <section className="premium-card-solid overflow-hidden rounded-[20px]">
          <div className="border-b border-token px-4 py-4 md:px-5">
            ...
            <p className="mt-2.5 text-[11px] text-secondary-text">
              Search also matches mapped category names for the selected shop.
            </p>
          </div>
                        ...
                        >
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center justify-center gap-1.5">
                              <button
                                type="button"
                                onClick={() => setViewingCategoryId(row._id)}
                              ...
                              <button
                                type="button"
                                onClick={() => openEditCategoryModal(row._id)}
                                className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-token bg-white text-secondary-text transition hover:bg-primary-soft hover:text-primary"
                                title="Edit"
                              >
      ...
      <MyCategoryViewPage
        open={Boolean(viewingCategoryId)}
        isModal
        mapId={viewingCategoryId}
        onClose={() => setViewingCategoryId("")}
        onEdit={
          viewingCategoryId
            ? () => {
                const currentId = viewingCategoryId;
                setViewingCategoryId("");
                openEditCategoryModal(currentId);
              }
            : undefined
        }
      />
      ...
      <CategoryFormModal
        open={Boolean(categoryFormModal)}
        mode={categoryFormModal?.mode ?? "create"}
        mapId={categoryFormModal?.mapId ?? ""}
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-token bg-white/95 px-4 py-3 backdrop-blur md:px-5">
          <div>
            <h2 className="text-base font-black text-slate-950">
              {mode === "edit" ? "Edit Shop Category" : "Add Shop Categories"}
            </h2>
            <p className="text-xs font-semibold text-slate-500">
              {mode === "edit"
                ? "Update the selected category mapping inside this popup."
                : "Select categories to map to this shop."}
            </p>
          </div>
