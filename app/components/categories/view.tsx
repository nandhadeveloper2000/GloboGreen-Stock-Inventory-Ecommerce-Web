...
export default function MyCategoryViewPage({
  open = true,
  mapId,
  isModal = false,
  onClose,
  onEdit,
}: MyCategoryViewPageProps = {}) {
  ...
  const handleEdit = () => {
    ...
    router.push(`/shopowner/categories/edit/${editId}`);
  };

  const imageUrl = data?.categoryId?.image?.url || "";
  const categoryName = data?.categoryId?.name || "Category Details";
  ...
  const content = loading ? (
    ...
    <div className={showAsModal ? "px-6 py-6" : ""}>
      {renderStateMessage({
        ...
        description: "The selected shop category record could not be loaded.",
        tone: "danger",
      })}
    </div>
  ) : (
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-100">
              {imageUrl ? (
                ...
                <img
                  src={imageUrl}
                  alt={categoryName}
                  className={showAsModal ? "h-56 w-full object-cover" : "h-72 w-full object-cover"}
                />
              ) : (
                <div
                ...
                >
                  <FolderTree className="h-14 w-14 text-slate-400" />
                </div>
              )}
            </div>
          ...
          <div className="space-y-5">
            <div className="rounded-card border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <Info label="Category Name" value={data.categoryId?.name || "-"} />
                <Info label="Category ID" value={data.categoryId?._id || "-"} />
              </div>
            </div>
