...
export default function MyBrandCreatePage() {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      const response = await apiClient({
        method: SummaryApi.brand_list.method,
      ...
      const rows: BrandItem[] = response.data?.data || [];
  ...
  const selectAllVisible = () => {
    setSelectedIds(availableBrands.map((item) => item._id));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient({
        method: SummaryApi.shopBrandMapBulkCreate.method,
        url: SummaryApi.shopBrandMapBulkCreate.url,
        data: {
      ...
      toast.success("Brands mapped successfully");
      router.push("/shopowner/my-brand/list");
    } catch (error: unknown) {
      toast.error(getErrorMessage(error, "Failed to map brands"));
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-4 md:p-6">
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-800">
          Please select a shop before creating my brands.
        </div>
      </div>
    );
  }
          ...
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-white/80">
                Select the active brands your shop can use for model mapping and
                product creation.
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={() => router.push("/shopowner/my-brand/list")}
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <button
                              ...
                              aria-label={
                                selected ? "Unselect brand" : "Select brand"
                              }
                            >
