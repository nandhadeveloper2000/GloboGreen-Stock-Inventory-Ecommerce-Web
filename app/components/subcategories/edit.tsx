...
export default function MySubCategoryEditPage({
  mapId,
  isModal = false,
  onClose,
  onSuccess,
  ...
  const id = mapId || (typeof params?.id === "string" ? params.id : "");
  ...
  async function handleSubmit() {
    if (!id) {
      toast.error("Invalid subcategory mapping id");
      return;
    }
    ...
    try {
      ...
      const api = SummaryApi.shopSubCategoryMapUpdate;
      ...
      toast.success("Subcategory mapping updated successfully");
  ...
  if (!data) {
    return (
      <div className={isModal ? "p-4 md:p-5" : "min-h-screen bg-slate-50 p-4 md:p-6"}>
        <div className={isModal ? "" : "mx-auto max-w-4xl"}>
  ...
  return (
    <div className={isModal ? "" : "min-h-screen bg-slate-50 p-4 md:p-6"}>
      <div className={isModal ? "space-y-4 p-4" : "mx-auto max-w-5xl space-y-5"}>
        {!isModal && (
            ...
            <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-white/80">
                  Update shop subcategory mapping status.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-[220px_1fr]">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <Info label="Subcategory" value={subCategory?.name || "-"} />
                <Info label="Category" value={category?.name || "-"} />
                <Info
                  label="Shop"
                  value={data.shopId?.shopName || data.shopId?.name || "Selected Shop"}
                />
              </div>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs font-medium text-slate-500">
                      Inactive subcategory will not be available in shop product
                      mapping.
                    </p>
                  </div>
