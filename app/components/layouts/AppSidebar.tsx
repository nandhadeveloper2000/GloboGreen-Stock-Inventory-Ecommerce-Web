...
function getIcon(label: string) {
  ...
  if (label.includes("Catalog")) return Layers3;
  ...
  if (label.includes("Compatibility")) return Wrench;
  if (label.includes("Approval")) return ShieldAlert;
...
function isWholesaleShop(shop?: ShopItem | null) {
  return normalizeShopType(shop?.shopType) === "WHOLESALE_SHOP";
}

function canShowMyShopProduct(shop?: ShopItem | null) {
  return (
    isWarehouseRetailShop(shop) ||
    isWholesaleShop(shop) ||
    normalizeShopType(shop?.shopType) === "RETAIL_BRANCH_SHOP"
  );
}
...
function formatShopType(value?: string | null) {
  const normalized = normalizeShopType(value);
...
export default function AppSidebar({ role }: AppSidebarProps) {
  ...
  const isRetailBranchShop = useMemo(
    () => normalizeShopType(selectedShop?.shopType) === "RETAIL_BRANCH_SHOP",
    [selectedShop]
  );
  ...
  const visibleItems = useMemo(() => {
    if (currentRole === "SHOP_OWNER") {
      return items.filter((item) => {
        if (item.label === "Shop Management") return canShowShopManagement(selectedShop);
        if (item.label === "My Shop Product") return canShowMyShopProduct(selectedShop);
        if (item.label === "Purchase") return canShowPurchase(selectedShop);
        return true;
      });
    }

    if (
      (currentRole === "SHOP_MANAGER" || currentRole === "SHOP_SUPERVISOR") &&
      isRetailBranchShop
    ) {
      return items.filter((item) => item.label !== "Purchase");
    }

    return items;
  }, [currentRole, isRetailBranchShop, items, selectedShop]);
