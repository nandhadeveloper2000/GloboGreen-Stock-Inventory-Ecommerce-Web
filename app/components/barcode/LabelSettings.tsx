...
export default function LabelSettings() {
  ...
  const fetchFormats = useCallback(async () => {
    ...
    try {
      ...
      setFormats(
        data.map((item) => ({
          ...item,
          fields: sanitizeLabelFields(item.fields),
        }))
      );
      ...
      if (data.length > 0) {
        ...
        setForm({
          ...defaultForm,
          ...active,
          fields: sanitizeLabelFields(active.fields),
        });
      } else {
  ...
  useEffect(() => {
    ...
    if (selectedFormat) {
      setForm({
        ...defaultForm,
        ...selectedFormat,
        fields: sanitizeLabelFields(selectedFormat.fields),
      });
    }
  ...
  const saveFormat = async () => {
    ...
    try {
      const payload = {
        ...form,
        fields: sanitizeLabelFields(form.fields),
        shopId,
      };
    ...
    popup.document.write(`
      ...
      <html>
        ...
        <body>
          <div class="label">
            ${
              form.fields.includes("PRODUCT_IMAGE") ||
            ...
            ${
              form.fields.includes("SELLING_PRICE") ||
              form.fields.includes("MRP") ||
              form.fields.includes("PURCHASE_DATE") ||
              form.fields.includes("EXPIRY_DATE")
                ? `
                  <div class="meta">
                    ${
                      form.fields.includes("SELLING_PRICE")
                        ? `<div><div class="meta-label">Selling</div><div class="meta-value">${form.currency} 18500</div></div>`
                        : ""
                    }
                    ${
                      form.fields.includes("MRP")
                        ? `<div><div class="meta-label">MRP</div><div class="meta-value">${form.currency} 19200</div></div>`
                        : ""
                    }
                    ${
                      form.fields.includes("PURCHASE_DATE")
                        ? `<div><div class="meta-label">Purchase</div><div class="meta-value">10 May 2026</div></div>`
                        : ""
                    }
                    ${
                      form.fields.includes("EXPIRY_DATE")
                        ? `<div><div class="meta-label">Expiry</div><div class="meta-value">31 May 2026</div></div>`
                        : ""
                    }
                  </div>
                `
