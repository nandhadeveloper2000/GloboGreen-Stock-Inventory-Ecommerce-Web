...
import CreateVendorPage from "./create";
import EditVendorPage from "./edit";
import ViewVendorPage from "./view";
...
type ModalState =
  | { type: "create"; vendorId?: never }
  | { type: "edit"; vendorId: string }
  | { type: "view"; vendorId: string }
  | null;
...
function CenterModal({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex min-h-dvh items-center justify-center bg-black/50 p-3 backdrop-blur-sm sm:p-5">
      <div
        className={classNames(
          "relative flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-2xl border border-token bg-white shadow-2xl",
          wide ? "max-w-6xl" : "max-w-4xl"
        )}
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-token px-4 sm:px-5">
          <h2 className="text-[15px] font-bold text-heading">{title}</h2>

          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-token bg-white text-secondary-text transition hover:bg-danger-soft hover:text-danger"
        ...
        <div className="flex-1 overflow-y-auto bg-soft-token/40 p-3 sm:p-5">
          {children}
        </div>
      </div>
    </div>
...
export default function VendorListPage() {
  ...
  return (
    <>
      ...
      {modal?.type === "create" && (
        <CenterModal title="Create Supplier / Vendor" onClose={closeModal} wide>
          <CreateVendorPage
            embedded
      ...
      {modal?.type === "edit" && (
        <CenterModal title="Edit Supplier / Vendor" onClose={closeModal} wide>
          <EditVendorPage
            embedded
      ...
      {modal?.type === "view" && (
        <CenterModal title="View Supplier / Vendor" onClose={closeModal} wide>
          <ViewVendorPage
            embedded
            vendorId={modal.vendorId}
            onClose={closeModal}
